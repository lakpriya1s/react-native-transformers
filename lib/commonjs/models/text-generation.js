 ...
 */
class TextGeneration extends _base.Base {
  outputTokens = [];
  ...
  async generate(tokens, callback, options) {
    ...
    const inputIdsTensor = new _onnxruntimeReactNative.Tensor("int64", initialTokens, [1, tokens.length]);
    ...
    while (lastToken !== this.eos && lastToken !== 32007n && sequenceLength < maxTokens && !this.stopGeneration) {
      ...
      feed.input_ids = new _onnxruntimeReactNative.Tensor("int64", BigInt64Array.from([lastToken]), [1, 1]);
      if (this.needPositionIds) {
        feed.position_ids = new _onnxruntimeReactNative.Tensor("int64", BigInt64Array.from([BigInt(sequenceLength)]), [1, 1]);
      }
    }
